...
const firebaseConfig = {
  ...
  databaseURL: "https://sport-f707c-default-rtdb.firebaseio.com",
  projectId: "sport-f707c",
  storageBucket: "sport-f707c.appspot.com",
...
/* ====== UID المشرف (ضع UID هنا بعد إنشاء حسابه في Firebase) ====== */
...
loginBtn.addEventListener('click', async ()=>{
  ...
  try{
    ...
    closeLoginModal();
  }catch(e){status.textContent='خطأ: '+e.message; console.error(e);}
});
...
function openRegister(){
  ...
  auth.createUserWithEmailAndPassword(email,pw)
    .then(()=>status.textContent='تم إنشاء الحساب. سجّل الدخول الآن.')
    .catch(e=>{status.textContent='خطأ: '+e.message; console.error(e);});
}
...
function renderProjects(arr){
  ...
  if(arr.length===0){projectsGrid.innerHTML='<p class="small">لا توجد مشاريع حتى الآن.</p>'; return;}
  arr.forEach(p=>{
